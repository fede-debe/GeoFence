...
class GeofenceBroadcastReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        // A Broadcast Receiver can receive many types of actions, but in our case we only care about when the geofence is entered. Check that the intent’s action is of type ACTION_GEOFENCE_EVENT.
        if (intent.action == ACTION_GEOFENCE_EVENT) {
            ...
            if (geofencingEvent.geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER) {
                ...
                // If the triggeringGeoFences array is not empty, set the fenceID to the first geofence’s requestId. We would only have one geofence active at a time, so if the array is non-empty then there would only be one for us to interact with.
